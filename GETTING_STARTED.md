...
### IntelliJ still shows `java: package org.junit.jupiter.api does not exist`

This project already resolves JUnit 5 from Maven. If the editor still highlights the import after a Maven reload, the quickest recovery path is:

```bash
# Rebuild the Maven model and test classpath
./mvnw clean test-compile
```

Then in IntelliJ:
1. Close the project
2. Re-open **from `pom.xml`** (not just the folder)
3. In the Maven tool window, use **Reload All Maven Projects**
4. Confirm `src/test/java` is marked as a **Test Sources Root**

If command-line Maven succeeds but the editor still shows red imports, the problem is local IDE metadata rather than a missing dependency in the project.

### Windows PowerShell command note

On Windows PowerShell, prefer the wrapper as `./mvnw.cmd` (or `.\mvnw.cmd`).
If you pass Maven `-D...` properties with dotted names, quote the whole argument:
